...
def evaluate_models(X_train, y_train, X_test, y_test, models, param):
    try:
        ...
        for i in range(len(list(models))):
            ...
            para = param[list(models.keys())[i]]  # ⚙️ Get parameters for GridSearch

            # 🔍 Hyperparameter tuning using GridSearchCV with 3-fold CV
            gs = GridSearchCV(model, para, cv=3)
            gs.fit(X_train, y_train)

            # 🛠️ Set the best found parameters to the model
        ...
        return report  # ✅ Return performance of all models
